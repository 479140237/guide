...
hexo.extend.helper.register('doc_sidebar', function(className){
  ...
  var module = this.page.canonical_path.split('/')[1];
  var result = '';
  var self = this;
  var prefix = 'sidebar.' + type + '.';
  ...
  _.each(sidebar, function(menu, title){
    ...
    _.each(menu, function(link, text){
      ...
      if (link.indexOf(path) > -1 && link.indexOf(module) === 0) itemClass += ' current';

      result += '<a href="' + (docRoot + link) + '" class="' + itemClass + '">' + self.__(prefix + text) + '</a>';
    })
  });
...
hexo.extend.helper.register('raw_link', function(path){
  return 'https://github.com/wdzjfe/guide/edit/master/source/' + path;
});
